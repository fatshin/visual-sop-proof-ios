# Demo script

1. Show the quality floor and evaluation CSV.
2. Select **Run analysis**.
3. Read the passing three-task macro-average, normalized bundle cost, and twenty-case count.
4. Compare the selected route with the GPT-5.6-only fixture baseline using the same units.
5. Explain the exact 20-case, one-task-per-case, bounded-quality, positive-baseline,
   duplicate-row, missing-model, and impossible-floor fail-closed conditions.
